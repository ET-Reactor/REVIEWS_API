...
module.exports = {
  ...
  getReviews: async (id, page, count, sort, cb) => {
    ...
    const reviews = await pool.query('SELECT id, rating, summary, recommend, response, body, date, reviewer_name, helpfulness FROM reviews WHERE product_id = $1 LIMIT $2 OFFSET (($3 - 1) * $1)', [prodID, numberCount, pageCount]);
    let returnObj = {
      product: prodID,
      page: pageCount,
      count: numberCount,
    ...
    for (let i = 0; i < reviews.rows.length; i++) {
      const photos = await pool.query('SELECT id, url FROM photos WHERE review_id = $1', [reviews.rows[i].id]);
      returnObj.results[i] = {
  ...
  getMeta: async (id, cb) => {
    ...
    const metaRtng1 = await pool.query('SELECT COUNT(rating) FROM reviews WHERE rating = 1 AND product_id = $1', [prodID]);
    returnObj.ratings["1"] = metaRtng1.rows[0].count;
    const metaRtng2 = await pool.query('SELECT COUNT(rating) FROM reviews WHERE rating = 2 AND product_id = $1', [prodID]);
    returnObj.ratings["2"] = metaRtng2.rows[0].count;
    const metaRtng3 = await pool.query('SELECT COUNT(rating) FROM reviews WHERE rating = 3 AND product_id = $1', [prodID]);
    returnObj.ratings["3"] = metaRtng3.rows[0].count;
    const metaRtng4 = await pool.query('SELECT COUNT(rating) FROM reviews WHERE rating = 4 AND product_id = $1', [prodID]);
    returnObj.ratings["4"] = metaRtng4.rows[0].count;
    const metaRtng5 = await pool.query('SELECT COUNT(rating) FROM reviews WHERE rating = 5 AND product_id = $1', [prodID]);
    returnObj.ratings["5"] = metaRtng5.rows[0].count;

    // query all the reviews that matches the product ID and get the recommend values
    const recommends = await pool.query('SELECT recommend FROM reviews WHERE product_id = $1', [prodID]);
  ...
  addReview: async function (reviewObj, cb) {
    ...
    const maxIDquery = await pool.query('SELECT max(id) from reviews');
    ...
    for (let i = 0; i < reviewObj.photos.length; i++) {
      const photosMax = await pool.query('SELECT max(id) from photos');
      const maxPhotoID = photosMax.rows[0].max + 1;
      await pool.query('INSERT INTO photos (id, review_id, url) VALUES ($1, $2, $3)', [ maxPhotoID, maxID, reviewObj.photos[i] ])
        .catch(e => {
          console.log(e);
        });
    }
  ...
  markHelpful: async function(r_id, cb) {
    // mongoDB/postgreSQL queries
    await pool.query('UPDATE reviews SET helpfulness = helpfulness + 1 WHERE id = $1', [r_id]);
    cb('success??');
  },
  ...
  reportReview: async function(r_id, cb) {
    // mongoDB/postgreSQL queries
    console.log(r_id);
    await pool.query('UPDATE reviews SET reported = true WHERE id = $1', [r_id]);
    cb('success???');
  },
